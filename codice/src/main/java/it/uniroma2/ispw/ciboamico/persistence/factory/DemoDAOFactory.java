...
// Factory DEMO: DAO in-memory con dati seed (utenti, prodotti, buoni).
...
public class DemoDAOFactory implements DAOFactory {
    ...
    private void seed() throws BusinessValidationException, DAOException {
        ...
        Prodotto miele = new Prodotto("Miele locale", 6.50, 20,
                LocalDate.now(ZoneId.systemDefault()).plusMonths(6), UnitaEnum.PEZZI, rv);
        Prodotto pomodori = new Prodotto("Pomodori", 2.20, 50,
                LocalDate.now(ZoneId.systemDefault()).plusDays(10), UnitaEnum.GRAMMI, rv);
        ...
        buonoDAO.save(new BuonoPromozionale("SALUTI20", rv,
                LocalDate.now(ZoneId.systemDefault()).minusDays(1), LocalDate.now(ZoneId.systemDefault()).plusDays(30),

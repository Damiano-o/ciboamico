...
// Controller dello Use Case estensione "Applica Buono Promozionale" (4a di UC-04).
...
public class ApplicaBuonoPromozionaleController {
    ...
    public OrdineBean applicaBuonoPromozionale(String codiceBuono, OrdineBean bean, UtenteBean utente)
        ...
        validaInput(codiceBuono, utente);
        bean.validate();

        BuonoPromozionale buono = trovaBuonoValido(codiceBuono, utente);
        Prodotto prodotto = prodottoDAO.findByNome(bean.getNomeProdotto());
        verificaVenditore(buono, prodotto);

        Utente compratore = recuperaCompratore(utente);
        double totaleScontato = buono.applicaSconto(prodotto.getPrezzo());
        registraBuonoComeUsato(utente, compratore, buono);
    ...
    private void validaInput(String codiceBuono, UtenteBean utente) throws BusinessValidationException {
    ...
    private BuonoPromozionale trovaBuonoValido(String codiceBuono, UtenteBean utente)
            throws BusinessValidationException, DAOException {
        ...
        if (utente.getEmail() != null && haGiaUsatoBuono(utente.getEmail(), codiceBuono)) {
            throw new BusinessValidationException(
                    UserErrorMessagesEnum.BUONO_GIA_USATO_MSG.message,
                    ExceptionMessagesEnum.BUONO_GIA_USATO.message,
                    "ERR-BUONO-GIA-USATO");
        }
        return buono;
    }

    private void verificaVenditore(BuonoPromozionale buono, Prodotto prodotto) throws BusinessValidationException {
    ...
    private Utente recuperaCompratore(UtenteBean utente) throws DAOException {
    ...
    private void registraBuonoComeUsato(UtenteBean utente, Utente compratore, BuonoPromozionale buono)
            throws DAOException {
        if (utente.getEmail() != null && compratore.getEmail() != null) {

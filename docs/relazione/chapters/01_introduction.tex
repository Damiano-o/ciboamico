...
\begin{itemize}
    \item \textbf{Utente (Client)}: manages the pantry and uses the recipe
    engine and the marketplace to buy products;
    \item \textbf{Venditore (Vendor)}: a local farmer or supplier who, once
    approved by the administrator, publishes products with prices;
    \item \textbf{Nutrizionista (Nutritionist)}: the only actor allowed to
    create recipes, which are published with a \emph{PROPOSED} state;
    \item \textbf{Amministratore (Administrator)}: approves vendors and
    recipes.
\end{itemize}

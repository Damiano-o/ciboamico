...
\begin{reqs}
    ...
    \item \textbf{FR-03:} The system shall notify the household about the products whose expiry date falls within the next three days from the current date, specifying the product name and the remaining days before expiry.
    \item \textbf{FR-04:} The system shall provide the details of a product associated with a barcode or a specific search query.
    ...
    marketplace only if the vendor is approved, the price is greater than
    zero and the expiry date is not in the past.
    ...
    ingredients and shall set its state to \emph{PROPOSED}.
    ...
    recipe.
    \item \textbf{FR-11:} The system shall authenticate a client, a vendor
    or an administrator by e-mail and password, managing a session for the
    ...
    defined in \textbf{BR-07}.
    \item \textbf{FR-13:} The system shall present to a vendor the orders
    received for their products, with their current state.
    \item \textbf{FR-14:} The system shall update the state of an order
    according to the transitions defined in \textbf{BR-04}, upon the vendor's
...
\begin{reqs}
    ...
    than zero.
    \item \textbf{BR-07:} An ingredient is equivalent to another when both
    belong to the same ingredient category defined by the system.
\end{reqs}
...
Use case \textbf{UC-10} is excluded from the evaluation scope and therefore
intentionally not mapped to any functional requirement.
